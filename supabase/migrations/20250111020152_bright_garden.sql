...
BEGIN
  ...
  IF NEW.status = 'pending' THEN
    -- Cancel if created more than 48h ago and within 24h of start
  ...
  ELSIF NEW.status = 'confirmed' AND NEW.status != 'paid' THEN
    -- Case 1: Created more than 48h ago and not paid 24h before start
    IF (CURRENT_TIMESTAMP - NEW.created_at > INTERVAL '48 hours') AND
       (NEW.start_time - CURRENT_TIMESTAMP <= INTERVAL '24 hours') THEN
      NEW.status := 'cancelled';
      NEW.updated_at := CURRENT_TIMESTAMP;
    
    -- Case 2: Created less than 48h ago and start time has passed
